...
def plot_results_decoder(
    ...
    if epoch == "Stimulus_ON":
        ...
        plot.fill_betweenx(np.arange(-1, 1.15, 0.1), 0, 0.4, color="grey", alpha=0.4)
    else:
        plot.set_xlabel("Time to go cue (s)")
        plot.fill_betweenx(np.arange(-1, 1.15, 0.1), 0, 0.2, color="grey", alpha=0.4)


def plot_results_session_summary(
    plot,
    df,
    colors,
    variables_combined=["WM_roll_1", "RL_roll_1"],
    y_range=[],
    x_range=None,
    epoch="Stimulus_ON",
    baseline=0.5,
    ...
    for color, variable, ax in zip(
        colors, variables_combined, np.repeat(plot, len(variables_combined))
    ):
        try:
        ...
        numeric_columns = df_loop.columns[
            df_loop.columns.to_series().apply(pd.to_numeric, errors="coerce").notna()
        ]

        real = np.mean(
            df_loop.groupby("session")[numeric_columns].mean(), axis=0
        ).to_numpy()
        ...
        if epoch == "Stimulus_ON":
            ...
            ax.fill_betweenx(np.arange(-1, 1.15, 0.1), 0, 0.4, color="grey", alpha=0.4)
        else:
            ax.set_xlabel("Time to go cue (s)")
            ax.fill_betweenx(np.arange(-1, 1.15, 0.1), 0, 0.2, color="grey", alpha=0.4)

        sns.despine()


def plot_results_session_summary_substract(
    fig,
    plot,
    df: pd.DataFrame,
    ...
    df_loop = (
        df.loc[(df["trial_type"] == variable)]
        .groupby("session")
        .mean()[numeric_columns]
        - df_shuffle.loc[(df["trial_type"] == variable)]
        .groupby("session")
        .mean()[numeric_columns]
    )
...
def add_to_summary(
    ...
    df_cum_iter["times"] = (
        df_iter.groupby("times").score.mean().reset_index()["times"].values
    )
    ...
    if substract == True:
        for iteration in df_iter.iteration.unique():
            df_cum_iter[iteration] = (
                df_iter.loc[
                    (df_iter.variable == variable_full)
                ...
                .mean()
                .values
                - df_iter.groupby("times")["score"].mean().values
            )
    else:
        for iteration in df_iter.iteration.unique():
            df_cum_iter[iteration] = (
...
def train_test_shuffle(
    ...
    for start, stop in zip(
        np.arange(initrange, endrange - r, r), np.arange(initrange + r, endrange, r)
    ):
        # print((start+stop)/2)
        if hit_train == "all" and variable_train == "all":
            ...
            df_train = df.loc[
                (df["state"] == variable_train[:2]) & (df.hit == hit_train)
            ]

        df_final, y = interval_extraction(
            df_train,
            decode=decode,
            align=epoch,
            start=start,
        ...
        if count_RL > 8 and variable_test == "WM_roll" and reduce_WM != False:
            if reduce_WM == "correct":
                count_RL = len(
                    df.loc[(df["state"] == "RL") & (df.hit == 1)]["trial"].unique()
                )
            elif reduce_WM == "all":
                count_RL = len(df.loc[(df["state"] == "RL")]["trial"].unique())
            else:
        ...
        if len(df_test.trial.unique()) <= 4 or len(df_test.vector_answer.unique()) != 2:
            print(
                "Not enough RL trials - within loop " + str(len(df_test.trial.unique()))
            )
            # real_score.append(0)
            break
        ...
        model = LogisticRegression(solver="liblinear", penalty="l2", C=0.99).fit(
            x_train, y_train
        )
        train_cols = df_final.columns
        ...
        for i in np.arange(nsurrogates):
            ...
            new_row = {
                ...
                "times": (start + stop) / 2,
                "epoch": epoch,
                "variable": (str(variable_test) + "_" + str(hit_test)),
            }
...
def train_test(
    ...
    for start, stop in zip(
        ...
        if hit_train == "all" and variable_train == "all":
            ...
            df_train = df.loc[
                (df[variable_train] > ratio_train) & (df.hit == hit_train)
            ]
        ...
        if count_RL > 4 and variable == "WM_roll" and reduce_WM != False:
            if reduce_WM == "correct":
                count_RL = len(
                    df.loc[(df["RL_roll"] > 0.4) & (df.hit == 1)]["trial"].unique()
                )
            elif reduce_WM == "all":
                count_RL = len(df.loc[(df["RL_roll"] > 0.4)]["trial"].unique())
            else:
        ...
        model = LogisticRegression(
            solver="liblinear", penalty="l2", C=0.99, class_weight="balanced"
        ).fit(x_train, y_train)
        train_cols = df_final.columns
...
def interval_extraction(
    ...
    df_final = df_final.pivot_table(
        index=["trial"], columns="cluster_id", values="fixed_times", fill_value=0
...
def synch_trial(df, T, upper_plot, lower_plot, trial=0, start=-2, stop=0):
    ...
    for i in range(surrogates):
        ...
        spiketrain = SpikeTrain(
            surrogate_spikes, units=ms, t_stop=stop_time, t_start=start_time
        )

        histogram_rate = time_histogram([spiketrain], 20 * ms, output="rate")
        times_ = histogram_rate.times.rescale(s)
        firing = histogram_rate.rescale(
            histogram_rate.dimensionality
        ).magnitude.flatten()
    ...
    df_spikes = pd.DataFrame(
        list(zip(cluster_id, FR_mean)), columns=["cluster_id", "FR"]
    )
